...
$files = @{
    ...
    'RocksmithAudioBridge.dll' = Join-Path $repo 'Installer/Resources/Release/RocksmithAudioBridge.dll'
...
foreach ($name in @('RSMods.exe', 'rsmodsplus.dll')) {
...
$proxyBytes = [IO.File]::ReadAllBytes($files['RocksmithAudioBridge.dll'])
if ($proxyBytes.Length -lt 64) { throw 'Invalid Audio Bridge proxy: RocksmithAudioBridge.dll' }
...
if ($proxyHeader -lt 0 -or $proxyHeader -gt $proxyBytes.Length - 6 -or
    ...
    throw 'The Audio Bridge proxy must be a valid x86 DLL: RocksmithAudioBridge.dll'
}
