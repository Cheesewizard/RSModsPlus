...
* **Drop Pedal**
  * Adds an in-game drop pedal from -24 to +24 semitones. With RS_ASIO, the ASIO engine shifts each configured player input before Rocksmith receives it, so audio and note detection move together. The Cable engine drives a MultiPitch pedal in the player's tone and transposes Rocksmith's active tuning reference so the tuner and scoring follow the audible shift. Authored non-A440 references are preserved. Enable it with `[Drop Pedal] EnableDropPedal = on` in RSMods.ini or the settings app (Tuning tab). Setup guides: [ASIO Drop Pedal](docs/asio-drop-pedal.md) and [Cable Drop Pedal](docs/cable-drop-pedal.md). Contributed by [@Cheesewizard](https://github.com/Cheesewizard).
...
&nbsp; | DropPedalToggleKey | Function keys (F1, F10) / Media Keys (Play/Pause, Stop, NextTrack, PreviousTrack) in Virtual Key format. | Toggles the drop pedal globally for both players. |
&nbsp; | DropPedalBaseTuningDownKey | Function keys (F1, F10) / Media Keys (Play/Pause, Stop, NextTrack, PreviousTrack) in Virtual Key format. | Lowers Player 1's physical base tuning for the readout. Hold Control for Player 2. |
&nbsp; | DropPedalBaseTuningUpKey | Function keys (F1, F10) / Media Keys (Play/Pause, Stop, NextTrack, PreviousTrack) in Virtual Key format. | Raises Player 1's physical base tuning for the readout. Hold Control for Player 2. |
**Drop Pedal** | &nbsp; | &nbsp; | &nbsp; |
