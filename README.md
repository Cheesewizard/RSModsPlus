...
* **Drop Pedal**
  * Adds pitch routing from -24 to +24 semitones. Drop Pedal mode shifts the guitar: with RS_ASIO, the ASIO engine shifts each configured player input before Rocksmith receives it, so audio and note detection move together, while the Cable engine drives a MultiPitch pedal in the player's tone and transposes Rocksmith's active tuning reference so the tuner and scoring follow the audible shift. Speaker Mode instead shifts Rocksmith's streamed music to the physical guitar tuning. Authored non-A440 references are preserved. Enable it with `[Drop Pedal] EnableDropPedal = on` in RSMods.ini or the settings app (Tuning tab). Guides: [ASIO Drop Pedal](docs/asio-drop-pedal.md), [Cable Drop Pedal](docs/cable-drop-pedal.md), and [Speaker Mode](docs/speaker-mode.md). Contributed by [@Cheesewizard](https://github.com/Cheesewizard).
...
&nbsp; | DropPedalPitchDownKey | Function keys (F1, F10) / Media Keys (Play/Pause, Stop, NextTrack, PreviousTrack) in Virtual Key format. | Lowers Player 1 by one semitone. Hold Control for Player 2. |
&nbsp; | DropPedalPitchUpKey | Function keys (F1, F10) / Media Keys (Play/Pause, Stop, NextTrack, PreviousTrack) in Virtual Key format. | Raises Player 1 by one semitone. Hold Control for Player 2. |
&nbsp; | DropPedalToggleKey | Function keys (F1, F10) / Media Keys (Play/Pause, Stop, NextTrack, PreviousTrack) in Virtual Key format. | Cycles Drop Pedal, Speaker Mode, and Off for both players. |
&nbsp; | DropPedalBaseTuningDownKey | Function keys (F1, F10) / Media Keys (Play/Pause, Stop, NextTrack, PreviousTrack) in Virtual Key format. | Lowers Player 1's physical base tuning for the pitch route. Hold Control for Player 2. |
&nbsp; | DropPedalBaseTuningUpKey | Function keys (F1, F10) / Media Keys (Play/Pause, Stop, NextTrack, PreviousTrack) in Virtual Key format. | Raises Player 1's physical base tuning for the pitch route. Hold Control for Player 2. |
**Drop Pedal** | &nbsp; | &nbsp; | &nbsp; |
&nbsp; | EnableDropPedal | on/off | Enables Drop Pedal and Speaker Mode. Off by default. |
&nbsp; | Engine | automatic/asio/cable | Selects the Drop Pedal input engine. Speaker Mode always uses Wwise music output. Automatic uses ASIO input shifting when available and Cable otherwise. Asio requires RS_ASIO and does not fall back. Cable always uses the in-game MultiPitch path. |
**Toggle Switches** | &nbsp; | &nbsp;
