...
namespace NoteByNoteHostServices
{
	namespace
	{
		...
		void __cdecl HostPublishExpectedAttackEvent(
			const NoteByNoteProtocol::ExpectedAttackEvent* source)
		{
		...
		void __cdecl HostLog(NoteByNoteProtocol::LogLevel level, const char* message)
		{
			const auto text = message == nullptr ? std::string() : std::string(message);
			switch (level)
			{
				case NoteByNoteProtocol::LogLevel::Debug:
					Logger::GetInstance().Log(text, LogLevel::Debug);
					break;
				case NoteByNoteProtocol::LogLevel::Info:
					Logger::GetInstance().Log(text, LogLevel::Info);
					break;
				case NoteByNoteProtocol::LogLevel::Warning:
					Logger::GetInstance().Log(text, LogLevel::Warning);
					break;
				case NoteByNoteProtocol::LogLevel::Error:
		...
		{
			if (out == nullptr || out->structSize < sizeof(NoteByNoteProtocol::RawToneEvidence))
			{
				return 0;
			}
		...
		using NoteByNoteProtocol::MlNoteVerdict;
		...
		uint8_t __cdecl HostQueryRawToneComb(double frequencyHz, NoteByNoteProtocol::RawToneComb* out)
		{
		...
		uint8_t __cdecl HostQueryRawNoteConfirmation(double frequencyHz, uint64_t minimumSampleIndex, uint64_t maximumSampleIndex, NoteByNoteProtocol::RawNoteConfirmation* out)
		{
			if (out == nullptr) return 0;
			*out = {};
		...
		{
			NoteByNoteProtocol::HOST_API_VERSION,
			sizeof(NoteByNoteProtocol::HostApi),
