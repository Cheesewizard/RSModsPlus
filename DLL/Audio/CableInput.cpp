...
namespace Audio::CableInput
{
	namespace
	{
		...
		// PortAudio v19 public structs in the 32-bit layout the game's PortAudio build uses.
		...
		// Output monitoring is off by default: wrapping the render client can stall start-up
		// before the profile screen (no PortAudio processing thread is created and one thread
		// stays parked inside a COM call). Enable it with RSMods.ini [Mod Settings]
		// MonitorOutput = on to diagnose "no sound"; every one-shot call on the wrapper then
		// logs, so a stall names the call it stopped at.
		...
		// Pinned so MMDevAPI's class (and the patched vtable) stays loaded for the process.
		...
		bool overlayEnabled = false;   // AudioDiagnosticsOverlay is opt-in (off by default)
		...
		// buffer per event, which is true of a real exclusive stream (buffer == period) but
		// not of the shared stream underneath: a shared engine buffer spans several packets,
		// so PortAudio would read a whole buffer per event, most of it stale, and the game
		// would hear scrambled audio. Serving exact chunks makes the contract hold on any
		...
		public:
				...
				UINT64* devicePosition, UINT64* qpcPosition) override
			{
				...
				// backlog in the ring would be served forever, one chunk stale, adding a full
				// period of latency that the stock polled path (which drains everything) does
				// not pay. Serve the newest whole chunk and drop the rest: a rare glitch on a
				// hiccup instead of permanent latency.
				...
				*frames = chunkFrames;
				...
				if (chunkQpc != 0 && presentedLayout.rate > 0)
				{
					...
					// The engine stamps a packet at the moment it receives it, i.e. at the packet's
					// end, so the mean sample age is the hand-off delay plus half the packet.
		...
		public:
				...
				REFERENCE_TIME bufferDuration, REFERENCE_TIME, const WAVEFORMATEX* format, LPCGUID session) override
			{
				...
				{
					// Not a PCM/float shape this client can present; hand the request to the
					// engine untouched (shared, so the cable still opens) and report the outcome.
				...
				const Attempt attempts[] =
				{
					...
					// A device whose Windows format differs in rate (for example a cable that
					// enumerates at 16 kHz on a fresh USB port) fails every rung above; asking the
					// engine to convert on the low-latency stream keeps the period instead of
					// dropping to the 22 ms floor.
			...
			HRESULT STDMETHODCALLTYPE GetBufferSize(UINT32* frames) override
			{
				...
				// shared stream underneath, whose engine buffer spans several packets: PortAudio
				// would process a whole buffer per event, most of it stale, and the game would
				// hear nothing usable. Present the packet size as the buffer so the assumption
				// holds.
				...
				WAVEFORMATEX** closestMatch) override
			{
				...
				if (!format) return E_POINTER;
				// Anything PCM/float this client can present is "supported": Initialize converts,
				// or has the engine convert, to reach it. PortAudio asks in exclusive mode for
				// the game's own 32-bit float mono 48 kHz, which is exactly the shape wanted.
			...
			HRESULT STDMETHODCALLTYPE GetService(REFIID riid, void** ppv) override
			{
				...
				// Always the mod's own capture object, even without conversion: AsioHook patches
				// the capture client's vtable once and refuses a second implementation, so every
				// input stream in the process must share one class. It serves exactly
		...
		private:
				...
				bool raw, bool autoConvert, UINT32& periodFrames, std::string& note)
			{
				...
				if (FAILED(hr) && autoConvert && deviceMixFormat.Format.nSamplesPerSec > 0 && engineFormat->nSamplesPerSec > 0)
				{
					...
					// is converting, ask in the device's own format and scale the frame count to
					// the stream's rate (a 16 kHz endpoint quoting 160 frames per 10 ms becomes
		...
		bool ReadModernInputSetting()
		{
			...
			char overlay[16] = {};
			GetPrivateProfileStringA("Mod Settings", "AudioDiagnosticsOverlay", "off", overlay, sizeof(overlay), iniPath.c_str());
			overlayEnabled = _stricmp(overlay, "on") == 0;
	...
	void Poll()
	{
		...
		// length, and the averaged figure, so the timestamp semantics can be checked from the
		// log.
		...
		if (measuredAny.load(std::memory_order_relaxed) && now - lastEvidenceTick >= 5000)
		{
			...
			// about 15.6 ms hand-off delay at the Windows default, about 1 ms when any process
			// holds a 1 ms timer. The event-driven modern path does not depend on it.
