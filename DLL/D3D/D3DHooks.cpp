...
namespace {
	...
	// Per-frame draw-hook state (2026-09-02, the Release FPS regression). All four draw
	// hooks called GameState::IsInSong() on EVERY draw call; that walks a pointer chain
	// into game memory, builds a std::string from the menu name and scans the song-mode
	// list, thousands of times per in-song frame. The answer cannot change mid-frame in
		...
		const void* userPointerVertexData = nullptr)
	{
		...
		// view-projection transform (the seq-631 quad shape from the 2026-08-22 frame), so
		// their stream-0 vertex data is the only place their position exists. Sample it.
		// Hook_DP reports no vertex count, so derive one from the primitive topology; its
		// startIndex parameter is the start vertex. Without this the entire non-indexed
		// draw path went unsampled (2026-08-22).
	...
	// Stale fretboard marker quad filter (2026-08-26, the doubles kill). The transition
	// captures identified the box's real draw: DrawIndexedPrimitive tri-strips from
	// 0xBEC0DA (inside FUN_00BEBCD0), stride 12, drawing marker quads from a per-frame
	// rebuilt dynamic buffer. Each quad is a 6-index degenerate-bridged block whose
	// vertex positions live in (x, stringHeight, fretCentre) space - captured live at
	// exactly RENDERED_FRET_CENTERS / RENDERED_STRING_HEIGHTS values. Every record,
	// placement and context write was falsified because none of them feed this stream;
	// the quads are re-emitted every frame from a source object that survives target
	// changes. So the filter acts at the draw itself: decode each quad block, keep the
	// current target's quads (and any block that does not decode onto the tables), and
	// re-issue only the kept runs - the same subset strategy the instance filter uses
	// for note heads. Active only while Note by Note holds a target.
	...
	// The batched glyph flush that draws the fingering NUMERALS (2026-08-26 evening:
	// a suppressed upcoming chord left its floating finger number behind - the "3" at
	// 3:7 during a C5 hold). One triangle-list draw carries ~21 glyph quads including
	...
	// Lock flags for the read-back (2026-09-02, the NBN-on frame cost). Measured in a Riff
	// Repeater hold: this filter alone cost ~7 FPS (52 -> 59 with it switched off) while
	// dropping nothing, because a plain READONLY lock on the game's DYNAMIC marker buffers
	// makes the runtime wait for the GPU to finish with them, ~3 such waits per frame.
	...
	// render frame (2026-08-31). The glyph flush may only drop a numeral that
	// ACCOMPANIES a dropped marker box: a stale marker and its floating finger number
	// come as a pair (the original bug: the leftover "3" at 3:7 beside its suppressed
	// box), but a LIVE note - a hammer-on's numeral, an upcoming note's number - also
	// legitimately sits on marker coordinates, and "decodes onto a marker coordinate
	// and is not the current target" wrongly condemned those (hammer-on numerals
	// vanished, 2026-08-31). Render-thread only, like the anchor table. The one-frame
	...
	// Draw suppression by runtime-selected mesh signature. The 0x00BEC0EE strip family and
	// the untextured view-projection quad (a +/-100000-unit ground plane by its vertices)
	// were both causally or analytically disproven on 2026-08-22, so the target signature
	// is no longer baked in: the bridge selects a stride/vertexCount/primitiveCount triple
	// and toggles it live, letting each remaining candidate be tested without a host
	// rebuild. Defaults off with an unset triple that matches nothing.
	std::atomic<bool> isPhysicalGuideSuppressionEnabled{ true };
	std::atomic<unsigned long long> physicalGuideConsideredDrawCount{ 0 };
	// The fretboard doubles fix (2026-08-25 night, the live elimination
	// campaign): the ghost fingering markers survive the native frozen mode,
	// every note-head render context, and every neck-placement site - they are
	// these physical marker meshes, and the 397-event capture measured their
	// signature (stride 32, 187 vertices, 104 primitives) with the target's
	// own marker sharing it. So suppression is DECODE-AWARE: a signature match
	// during a hold decodes the draw's own string/fret from the shader
	// transform (the same decoder the capture uses) and suppresses only
	// NON-TARGET markers - the target's fingering stays, colours stay.
	// Ships ENABLED per the test policy; guide-suppress-off is the revert.
	...
	// Host-drawn finger numerals (2026-08-26 evening): the game's own numeral glyphs
	// are screen-space quads that never repaint on frozen retargets (transition
	// capture proof), so the overlay draws the template's fingers itself. Anchors are
	// collected here at draw time: a kept member's fingering-panel/ring/mini-quad
	// draw carries the marker's world position in c0..c3's translation and the
	// view-projection in c4..c7 (screen-map capture, atlas
	// docs/investigations/data/screenmap: fretboard-anchored draws put the axis-swap
	// world matrix in c0..c3 and the same row-convention VP that note heads carry in
	// c0..c3 into c4..c7). Render-thread only, like the suppressors around it.
	...
	// Projection convention, settled by the 2026-08-26 late-evening register probe
	// (probe_anchor_probe, atlas nbn-enable-crash folder): at the FINGERING-PANEL
	// draw (32/187/104) the fretboard camera's view-projection sits in c4..c7,
	// candidate-A rows (clip_i = dot(c(4+i), world)); the computed screen point for
	// the 3:5 panel landed within ~33px of the actual marker. The two earlier
	// misfires: v1 also collected anchors from ring/mini-quad draws whose shaders
	// carry DIFFERENT c4..c7 (last writer overwrote the table with garbage), and
	// v2's note-head c0..c3 is the HIGHWAY camera, not the fretboard camera. So:
	// anchors come from panel draws ONLY, through that draw's own c4..c7.
		...
		int fret)
	{
		...
		// viewport (1920x1080 on Philip's rig) is smaller than the window/backbuffer
		// (2560x1600) the overlay's font draws into, which put the first correct
		// projection at 75% scale toward the top-left. Fractions cancel the viewport
		// out; the overlay multiplies by its own window size.
		FingerAnchor anchor;
		...
		UINT primitiveCount)
	{
		...
		// primitives, indexed). The stride-12/100-primitive class was BRIEFLY
		// suppressed here too (2026-08-25 night) and promptly ate note-trail
		// segments - trails share the signature AND decode onto string/fret
		// coordinates because they lie along strings. The neck note marker that
		// class was aimed at draws through DrawPrimitiveUP instead; its
		// suppression lives in Hook_DPUP with the same decode gate.
		//
		// 2026-08-26 afternoon: two companion families joined the list, found by a
		// mid-hold capture after Philip reported half-suppressed previews (finger
		// RINGS without their panels at the upcoming chord's coordinates): the
		// stride-12/8-vert/6-prim ring and the stride-32/4-vert/2-prim marker
		// mini-quad. Both are fretboard-anchored (the decode's longitudinal limit
		...
		// Glyph guard (2026-08-31): HUD/menu text quads share the mini-quad signature
		// (a single glyph IS a stride-32 4-vertex 2-primitive quad), and screen-space
		// text draws without a vertex shader - so GetVertexShaderConstantF below reports
		// the LAST shader draw's constants, which can decode as a perfectly plausible
		// marker. The deterministic draw order made the SAME letters vanish every frame
		// ("PEED" / "MI ED" / "Settin s"). A draw with no vertex shader bound cannot be
		// a fingering marker; never suppress it.
		...
		// The keep set replaces the bare single target (2026-08-26): chord holds
		// cleared the target and left this suppressor dormant, so the successor
		// chord's fingering panels painted during chords - the same disease the
		// stale-marker quad filter had. Note holds keep target + legato group;
		...
		// Keep-set filtering for chord holds too (reverted 2026-08-26 evening from a
		// brief drop-everything policy): dropping the whole layer took the member
		// BOXES with it and left the fretboard empty, which reads worse than
		// panels whose numerals are coordinate-matched (and usually correct, since
		// coinciding coordinates tend to share fingers). The authoritative
		// fingering rides the overlay line either way. Philip's UX rule of record:
		// in note-by-note, repeated asks should RE-SHOW the chord, never blank it.
		for (int keep = 0; keep < keepCount; ++keep)
		{
...
void D3DHooks::FinishNoteByNoteRenderFrame()
{
	...
	// Cross-thread corruption lattice (2026-08-27): the crash campaign's death
	// point wanders between fixed scoring-path seams, which fits a second thread
	// racing the frozen choreography. This is the render-thread arm: validate the
	// heaps every frame for the first ~900 frames after each Note by Note enable,
	// so a corruption arising on or near this thread names a render seam instead
	// of the next scoring seam. Uses the HOST's copy of HeapCheckpoint.
...
/// <summary>
/// IDirect3DDevice9::DrawPrimitiveUP Middleware. Observe-only: user-pointer draws were a
/// blind spot for the bounded physical-marker capture until 2026-08-22.
/// </summary>
HRESULT APIENTRY D3DHooks::Hook_DPUP(IDirect3DDevice9* pDevice, D3DPRIMITIVETYPE PrimType, UINT PrimCount, CONST void* pVertexStreamZeroData, UINT VertexStreamZeroStride) {
	if (isInSongForDrawHooks.load(std::memory_order_relaxed))
	{
		...
		// relaxed (2026-08-25 night; this UP path was the capture's historical
		// blind spot and carries no other suppression). Same rules as the
		...
		if (isPhysicalGuideSuppressionEnabled.load(std::memory_order_acquire)
			...
			&& PrimCount == 100)
		{
			// Keep set instead of the bare single target (2026-08-26), so this gate
			// stays live during chord holds too - see ShouldSuppressPhysicalGuideDraw.
			int keepStrings[8];
			int keepFrets[8];
			int keepCount = 0;
			if (NoteByNoteHighwayRenderer::TryGetMarkerKeepCoordinates(
...
HRESULT APIENTRY D3DHooks::Hook_Reset(IDirect3DDevice9* pDevice, D3DPRESENT_PARAMETERS* pPresentationParameters) {
	...
	// Device-reset forensics (2026-09-02). A Reset that keeps returning D3DERR_INVALIDCALL
	// (0x8876086C) is D3D9 saying a D3DPOOL_DEFAULT resource is still referenced or a buffer
	// is still locked; the game then retries every frame with a frozen picture, which is
	// exactly the "hung after unpause" of 2026-09-01 (main thread looping through this
	// function, Reset -> INVALIDCALL, sampled live). Logged so the next occurrence carries
	// the code and the count instead of needing a live stack sample.
	{
...
HRESULT APIENTRY D3DHooks::Hook_DIP(IDirect3DDevice9* pDevice, D3DPRIMITIVETYPE PrimType, INT BaseVertexIndex, UINT MinVertexIndex, UINT NumVertices, UINT StartIndex, UINT PrimCount) { // Draw things on screen
	...
	// Idle-path budget (2026-09-02, the Release FPS regression). With Note by Note holding
	...
	// The doubles kill: quad-level filtering of the stale fretboard marker slices,
	// identified by the 2026-08-26 transition captures. Runs after the research
	...
	if (isInSong)
	{
		...
		// Decode-aware signature suppression on the INDEXED path (2026-08-26
		// afternoon): the fingering panels, finger rings and marker mini-quads all
		// draw through DrawIndexedPrimitive, but this suppressor was only ever
		// consulted from Hook_DP (with vertexCount 0), so the panel signature could
		// never match and off-keep-set fingering previews survived every hold. The
