...
namespace
{
	...
	// The sub-element placement setups of the note-visual class (vtable 0x11CF1D8,
	// handover-5 addendum 12): run-once state machines, one per quad type, that anchor a
	// quad on the neck. The note visual (sub-element +0x8) carries its string at +0xC and
	// fret at +0xD as bytes. All five slot functions share the __fastcall(subElement)
	// shape; which of them places the visible read-ahead marker is what the dry log
	// answers (the first build hooked only 0x7A8B10 and it never fired in Learn a Song).
	// Sites 0x7A8CE0/0x7A8E90 proved to place the HIGHWAY visuals (relocating them
	// stripped the read-ahead highway, 2026-08-18); the fretboard markers come from one of
	// the later slots. All __fastcall(subElement) siblings are hooked; which sites target
	// mode may act on is the runtime site mask, so highway sites are never relocated again.
	...
	// The real lever (handover-5 addendum 12 follow-up): FUN_0079D070 is the per-frame
	// marker updater - it computes "is the current time inside this note's active window"
	// via FUN_007E9ED0(visual+0x38) and, on change, rebroadcasts the lit/dim bank to the
	// marker quads. With the transport frozen, upcoming notes' approach windows contain the
	// frozen time, so their markers light early: the bug is that one comparison. The
	// updater knows its visual (string at +0xC, fret at +0xD); the condition is the seam.
	// The updater detour parks the visual in a thread-local, and the condition detour
	// forces the "outside window" answer for non-whitelisted visuals while a hold is
	// owned. Relocation (above) is retained for diagnostics but proved wrong for this:
	// visual position is the HIGHWAY position, and moving it stripped the highway twice.
	...
	// Presentation suppression is OFF.
	//
	// Philip's decision after playing the suppressed build: Note by Note should look like
	// Rocksmith+'s note-by-note mode and keep the upcoming highway visible, because a
	// player still learning the song needs to read ahead. Hiding everything except the
	// target was an investigation, not the feature. Two of the visual faults reported
	// against the suppressed build, incorrect bend lines and wrong trails, are most likely
	// caused by suppression starving per-note state rather than being separate bugs, and
	// the complaint that the next note "comes in early" only reads as wrong because its
	// neighbours are hidden.
	//
	// Everything the investigation established is kept and still works. Setting this to
	// true re-engages both mechanisms: the descriptor gate at 0x7A4B50 and the D3D instance
	// filter. Flow Mode will need the full highway too, so suppression must stay a Note by
	// Note policy rather than something tied to controller activity.
	//
	// Known cost of leaving it off: the frame-rate fix came from suppressing the draw,
	// because the frozen transport keeps the whole remaining chart resident. If the frame
	// rate regresses, the lever is the noteway pool gate at 0x7E22F0, which controls
	// residency, rather than the draw gates, which control appearance.
	// Superseded by the runtime HighwayMode. Kept as the default the mode initialises to, so
	// behaviour out of the box is unchanged.
	...
	// The fretboard is gated separately from the highway, and this is the whole point.
	//
	// Rocksmith assumes time always advances. The near pool in 0x7E2340 holds every note
	// inside a time window, and 0x7A5CE0 admits a note to the neck diagram while
	// currentTime is before its end, so the window sweeps and notes reach the fretboard one
	// at a time. Note by Note freezes currentTime, which does not collapse that window to a
	// single note, it just stops it moving. Two notes a tenth of a second apart therefore
	// sit on the fretboard together, permanently, and Philip could not tell which to play.
	//
	// Gating 0x7A5CE0 was tried earlier and dismissed because it "changed nothing on the
	// highway". That was the right gate judged against the wrong goal: it drives the
	// fretboard grid written by 0x7AA140 and nothing else. Now that the highway is
	// deliberately unfiltered so the player can read ahead, that is exactly the separation
	// wanted: the neck diagram shows only the current gesture, the highway shows everything.
	//
	// The target's published legato group stays visible here too, so a hammer-on run reads
	// as one gesture on the fretboard rather than appearing a note at a time.
	...
	// The epoch alone misses the hybrid-release rebuild (StartAt core + coordinated restart),
	// which rebuilds the note pool WITHOUT incrementing the epoch and crashed on a chord slide
	// (crash-evidence/2026-08-27-chordslide-execfault: execute fault, the game called a freed
	// chord panel's vtable). So the settle also arms on the scoring target changing and on the
	// native hold releasing - i.e. on ANY hold hand-off - since every such transition is a
	// moment Rocksmith may retire an object the draw path still references.
	...
	// Chord-hold shape for the stale-marker quad filter (2026-08-26). Chord holds
	// deliberately clear the single-note target (suppressing a chord's siblings would
	// hide notes the player must play), which left the quad filter dormant during
	// chords and future markers visible. The overlay's event handler already reads the
	// SNG chord template's per-string frets for the target text, so it stashes them
	// here; the filter keeps exactly these coordinates while the live state confirms
	// the same chord is held. Written on the scoring thread, read on the render thread
	// under the same tolerated-race rules as the single-target fields above.
	...
	bool __cdecl IsOutsideCurrentGesture(void* note)
	{
		...
		// A chord's member notes are separate records sharing the chord's onset. The
		// record-keyed whitelist above hid them, which broke the chord shape apart on
		// the fretboard during chord holds (Philip, 2026-08-24: "looks like this chord
		// has missing notes"). Same authored time as the target = same gesture.
	...
	//   007a4a85  CALL 0x7A52C0     ; draw   (push ESI=owner, push EDI=note, 2 floats)
	//   007a4a93  CALL 0x7A6160     ; draw   (note in EAX, one float)
	//
	// Two earlier attempts intervened above this split and both were wrong. Feeding a
	// far-future time made the engine take its own already-past branch, but that branch
	// still runs the two state calls, so hidden notes accumulated garbage geometry and
	// their trails overshot once visible. Returning early skipped all four, which
	// starved the state instead: notes came back grey, with wrong line lengths, and
	// flickered as they reappeared.
	//
	// Gating only the two draw calls leaves the state pair running every frame with
	// real values, so a hidden note stays fully maintained and simply is not drawn.
	//
	// That last paragraph was the working theory, and the counters below disproved it.
	// A deployed build logged, consistently across many targets:
	//
	//   geometry 848/911 suppressed, submit 848/911 suppressed, group=0
	//
	// So the detours fire (~14 notes per frame), suppression triggers on ~13 of them
	// (the ~1 survivor being the target), and the notes are drawn on screen anyway.
	// 0x7A52C0 and 0x7A6160 are therefore NOT the highway draw path, whatever their
	// names suggest. Skipping all four calls in 0x7A4A40 did clear the highway, and
	// skipping only the last two does not, which puts the real drawing in the first
	// two. 0x7A4B50 iterates the note's presentation descriptors, so it is gated here
	// and 0x7A4AA0 is deliberately left running: skipping both starved per-note state
	// and produced grey notes, wrong trail lengths and reappearance flicker.
	//
	// ABIs read from the running image (PID 21628), not inferred:
	...
	// 0x7A4AA0 is deliberately NOT gated. This records why, so it is not retried.
	//
	// It was gated once, on the theory that it drew the sustain and bend trails that
	// survived the descriptor gate. Its ABI is known and correct: the note arrives in EDI
	// rather than on the stack (007a4aa4 MOV ESI,[EDI+0x154]), and its single return is
	// RET 0xC at 0x7A4B41. Field note+0x154 is the NoteVfx controller identified during the
	// 2026-08-14 prompt investigation.
	//
	// The detour installed and suppressed at exactly the expected rate, logging
	// vfx 1543/1581 beside descriptors 1543/1581, and the trails were visually unchanged.
	// It also contributed to state starvation: with Note by Note switched OFF, so nothing
	// was being suppressed at all, the entire highway rendered grey. That is per-note visual
	// state which was never built while those notes were suppressed.
	//
	// The general lesson for this layer: 0x7A4AA0 and 0x7A4B50 populate per-note visual
	// state, they are not draw calls. Suppressing them prevents future updates but cannot
	// remove geometry that is already built, which is why the visible result is timing
	// dependent, and starving them leaves a note with no colour when it is later revealed.
	// Hiding a note belongs at the D3D instance layer, where filtering is non-destructive.
...
namespace
{
	...
	// Fade-path instrumentation (the fretboard-marker hunt, 2026-08-19 late). The
	// live mode-4 falsification proved the updater/broadcast chain is highway-side
	// and the FRETBOARD marker's per-frame lit lever is unidentified. Lead
	// candidate is the fade pipeline. Armed by the bridge (-FadeDry, budget 150,
	// re-armable), independent of the placement mode so mode 3's presentation
	// stays exactly as observed while sampling. Logs sites 2 (color step: which
	// paths run and with what flag/state), 4 (fade batch list sizes) and 5 (fade
	// controller: the (ctx+0xC)+0x68/+0x6C/+0x70 fade config against the
	// element's string/fret), tagged (NBN FADE) for correlation against which
	// markers are visibly lit.
	...
	// The stale-marker fix, proven live on 2026-08-22: native note progression clears the
	// element state bytes at sub+0x50/0x51 when a note passes (memory watches showed 17
	// on/off transitions during natural play and zero clears across Note by Note
	// retargets), and clearing those two bytes on a live stale element removed its neck
	// marker immediately while the selected target's marker stayed.
	//
	// The stale-marker expiry policy lives in the reloadable probe (see
	// ObserveNeckPlacementStep in ResearchProtocol.hpp): three host-side write policies
	// against the element state bytes faulted the scoring commit, and iterating on that
	// policy must not cost a game restart. The host's only job here is forwarding the
	// placement steps.
	...
	// Stale-marker dim (v7.8). A played note leaves the spawner's per-frame update set,
	// so its marker keeps the last broadcast state forever while the transport is frozen
	// (measured: no window checks for onsets behind the playhead). The fix: the updater
	// FUN_0079D070 keeps the note visual in EDX across the condition call, so the naked
	// thunk passes EDX through and the helper remembers the TARGET's visual while it is
	// queried each frame. When the target advances, ONE synthetic call of the native
	// updater on the remembered visual makes the game re-ask the condition - now
	// answered "outside" because the onset no longer matches the new target - and the
	// game's own broadcast loop dims its own marker from its own color bank.
	...
	// Identity word for the remembered visual (2026-08-31 hardening): the onset float at
	// +0x38 is the only revalidation the dim had, and a freed allocation reused by a
	// rebuilt visual can carry a matching onset. Every legitimate updater visual shares
	// vtable 0x11CF1D8 (the whitelist survey + live PLACEMENT logs), so the vtable is
	// captured with the pointer and must still match at dim time. The destructor-detour
	// eviction alone is not enough: it is a probe-requested hook, so every probe reload
	// opens a gap where a dying visual is not evicted - the mechanism that left the
	// persistent neck's fret-number elements dimmed for a whole game session (the
	// "removed frets" bug; only a game restart rebuilt them).
	...
	// Past-onset dim list (2026-09-01, the doubled-note session). The dense-advance
	// coordinated rebuild destroys the remembered previous-target visual before its
	// deferred dim can fire: a full session logged "stale marker dropped" on every
	// advance and "dimmed" on none - the identity hardening above CORRECTLY refused
	// the dead pointer, which left every advanced-past marker lit (the pre-v7.8
	// artifact back in a new costume). The game re-creates a visual for the played
	// note in the rebuilt pool and queries it again (live PLACEMENT logs show window
	// checks for onsets behind the target in the current build), so the capture is
	// generalized: any identity-passing updater visual whose onset lies BEHIND the
	// target is a stale candidate, caught on its next natural query after the
	// rebuild. Each onset dims once; the list resets when the target moves backward
	// (section wrap / re-arm), exactly one slot is in flight at a time, and the
	// existing deferred-fire path with its onset+vtable revalidation does the actual
	// dim.
	...
	// Upcoming-marker dim (v8, 2026-08-22 evening; v8.1 same night). The remaining
	// artifact: a note whose approach window opens during a frozen hold reveals a LIT
	// fretboard marker beside the target. Three falsified routes closed the color side
	// (no second draw; the state bytes fault the commit; the color-step contexts never
	// apply under Note by Note in any phase - probe policy v6/v7). The one lever with a
	// live-proven marker dim is the stale-marker mechanism above: a synthetic
	// native-updater call whose condition answer comes back OUTSIDE.
	//
	// v8 lesson (first live A/B): the condition is a SECTION check, not an approach
	// window - every in-section upcoming note answers inside - and holding dimmed onsets
	// forced-outside for the whole hold grayed a highway of note faces. So v8.1 dims each
	// upcoming note's marker ONCE per hold and immediately releases it: the synthetic
	// call's own query is the only forced answer (it must be, or nothing changes and no
	// broadcast fires), and the next natural updater pass flips the answer back inside,
	// relighting the highway face. The fretboard marker stays dim through that relight -
	// the asymmetry mode 3 already relies on: dimmed previous-target markers survive
	// natural inside answers between holds without re-lighting, and mode 4 proved lit
	// broadcasts do not drive the marker. Dimming every in-section upcoming marker is
	// the intended presentation: only the target lit on the physical fretboard.
	...
	// Runtime toggle for the dim itself (updim-on|off; see the hpp note): off =
	// upcoming notes keep their colour while frozen, at the risk of the
	// double-marker bug the dim originally fixed. DEFAULT OFF (Philip,
	// 2026-08-25 night): the dim masks the native sweep instead of fixing it,
	// and the policy is features-under-test run enabled with reality exposed -
	// the double markers are a native problem to fix natively, not to hide.
	volatile bool isUpcomingDimEnabled = false;

	// v8.2's reveal detection was removed after its own diagnostics falsified it
	// (2026-08-22): site 2 only sees PENDING contexts - the visible marker's context
	// completed long ago and left the walk - and the element state bytes read an
	// identical (0,1) for showing and non-showing markers, so no reveal signal exists at
	// that seam. elem+0x04 IS the note onset in the condition's timebase (verified from
	// the same diagnostics). A future colored-highway version needs the draw-event
	// route: the physical-marker draw capture already decodes string/fret from marker
	// draws, and a non-target marker draw during a steady hold is the true reveal.
		...
		void* edxVisual, uint32_t returnAddress)
	{
		...
		{
			// The stale-marker capture deliberately SURVIVES ownership gaps. A safety
			// release (Philip fought a bend to the timeout, 2026-08-18) drops ownership
			// between holds, and clearing here left the played note's marker lit into
			// the next hold with nothing to reconcile. The capture is safe to keep: the
			// destructor detour evicts it if the visual dies, and the reconcile
			// revalidates the onset (and drops on backward motion) before dimming.
			return result;
		}
		// Forcing spans EVERY phase of an owned hold (task #1, 2026-08-18: Philip saw the
		// extras flip lit for several frames between holds, because the transition phases
		// handed all queries back to native). The 16:39 crash (dump 90332 at 0x78B73A)
		// that motivated the earlier phase==Holding guard hit v7.6's UNCONDITIONAL forced
		// mode, which kept forcing through the practice loop's natural restart; a restart
		// releases the hold, so the ownsNativeHold gate above already restores native
		// answers on that path. If dense-advance rebuilds glitch under forcing, narrow
		// this to exclude the rebuild phases specifically rather than restoring the
		// blanket Holding guard.
		...
		if (isSteadyHold && syntheticDimActive == 0)
		{
			...
			{
				// A query for a PAST note (2026-09-01): the rebuild-survivor path. The
				// game is updating a visual for a note behind the target - a marker the
				// per-advance mechanism above can no longer reach, because the visual it
				// remembered died in the rebuild. Capture THIS live visual as the stale
				// candidate; the deferred fire above dims it on the next query with the
				// full onset+vtable revalidation. One in flight at a time; each onset
				// dims once per section.
			...
			if (upcomingCandidateVisual != 0)
			{
				...
				&& !IsGroupOnset(state, time, ONSET_TOLERANCE_SECONDS))
			{
				// Beta presentation (Philip's call, 2026-08-22): every upcoming
				// in-section note's marker dims and stays dim, accepting the gray
				// upcoming highway faces. v8.2's reveal scoping was falsified by its
				// own diagnostics: site 2 only sees PENDING contexts, the visible
				// marker's context completed long ago, and the element state bytes
				// read identically (0,1) for showing and non-showing markers - there
				// is no reveal signal at that seam. The draw-event route (the
				// physical-marker draw capture decodes string/fret) remains the
				// documented path to a colored-highway version. Capture the visual
				// for one synthetic dim on a later query.
				float edxOnset = 0.0f;
				uintptr_t edxVtable = 0;
		...
		// Whitelist history (ticket #46 and both regressions, all 2026-08-18):
		// blanket TARGET-ONLY grayed every approaching highway note; blanket
		// TIME-DIRECTIONAL re-lit upcoming fretboard string markers; splitting by
		// CALLER (updater strict, initializers directional) fixed the markers but
		// re-grayed the highway the moment a hold latched, because the per-frame
		// updater's answer lands in the shared flag byte at visual+0x50 that the
		// color pipeline step reads for BOTH surfaces (the static decompile chain,
		// 2026-08-19, closed what the vtable survey could not: every updater
		// visual shares vtable 0x11CF1D8, but the SURFACES are told apart by the
		// color step's ctx+0x28, not by the visual). Mode 3 keeps the shipped
		// time-directional interim. The caller-split and color-step flag-swap path
		// were removed after the live 2026-08-19 A/B falsified them: strict updater
		// answers gray the highway while the fretboard marker ignores the updater.
		// The bounded fade-path capture below now targets the unidentified lever.
	...
	{
		// 0x7A9220 is the marker sub-element's deleting destructor (decompiled 2026-08-18:
		// vtable swap, resource releases, optional free - not a visibility setter as
		// earlier guessed). That makes it the eviction signal for the stale-marker
		// capture: if the dying sub-element belongs to the remembered visual, forget it.
...
bool NoteByNoteHighwayRenderer::InstallPresentationGate()
{
	...
	// The FUN_0079D070 updater stays UNHOOKED: its custom convention (stack parameter plus
	// EDX side-channel) crashed v7.3 on song load (dump Rocksmith2014.exe.88372.dmp, fault
	// at 0x78B73A). v7.5 needs only the condition, whose __thiscall(this, float) contract
	// is verified, and identifies notes by onset time instead of via the updater's visual.
